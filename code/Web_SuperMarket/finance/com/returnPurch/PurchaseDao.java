...
public class PurchaseDao extends ConnectionAware{
	...
	public List<Purchase> selectToday(String time){
		...
		try {
			PreparedStatement ps=conn.prepareStatement("select sum(returnCount) as SaleNumber from return_tab where returnTime=?");
			ps.setString(1, time);
	...
	public float selectMoney(String time){
		float money=0;
		try {
			PreparedStatement ps=conn.prepareStatement("select sum(returnPrice*returnCount) as money from return_tab where returnTime like '"+time+"%'");
			ResultSet rs=ps.executeQuery();
			if(rs.next()){
	...
	public List<Purchase> selectTurnoverYear(String time){
		...
		for(int i=1;i<=12;i++){
			...
			try {
				PreparedStatement ps=conn.prepareStatement("select sum(returnCount) as SaleNumber from return_tab where returnTime like '"+FinalTime+"%'");
				ResultSet rs=ps.executeQuery();
				if(rs.next()){
					Purchase purchase=new Purchase();
					purchase.setCount(rs.getInt("SaleNumber"));
	...
	public List<Purchase> selectTurnoverMonth(String time){
		...
		for(int i=1;i<=day;i++){
			...
			String sql = "select sum(returnCount) as SaleNumber from return_tab where returnTime like '"+FinalTime+"%'";
			try {
	...
	public List<Purchase> selectOne(String time){
		List<Purchase> purchaseList=new ArrayList<Purchase>();
		try {
			PreparedStatement ps=conn.prepareStatement("select productName,productTypeName, sum(returnPrice*returnCount) as money,sum(returnCount)as count from return_tab inner join ((productType_tab inner join product_tab on productType_tab.productTypeId=product_tab.productTypeId) inner join purch_tab on purch_tab.productId=product_tab.productId) on return_tab.purchId=purch_tab.purchId  where returnTime = ? group by productName,productTypeName");
			ps.setString(1, time);
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
				Purchase purchase=new Purchase();
	...
	public List<Purchase> selectOne1(String time){
		...
		List<Purchase> purchaseList=new ArrayList<Purchase>();
		try {
			PreparedStatement ps=conn.prepareStatement("select productName,productTypeName, sum(returnPrice*returnCount) as money,sum(returnCount)as count from return_tab inner join ((productType_tab inner join product_tab on productType_tab.productTypeId=product_tab.productTypeId) inner join purch_tab on purch_tab.productId=product_tab.productId) on return_tab.purchId=purch_tab.purchId  where returnTime '"+time+"%' group by productName,productTypeName");
		//	ps.setString(1, time);
			ResultSet rs=ps.executeQuery();
			while(rs.next()){
